...
/// guest-reported display name (sanitized by the router), and the key handle the op created or
/// removed (the minted handle on a GENERATE, the deleted handle on a DELETE, else nil), which
/// lets the UI keep a live per-app key count. It never receives the token or any key material.
